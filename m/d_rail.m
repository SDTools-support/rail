...
PA.ms % current slice, see sdtweb var.ms

if sdtweb('_TutoNeed','figgen')
 % xxxeb2gm : PA.ms and PA.mt commonly used in dynavoie
...
% keywords{var{},fcn{fe_homo.DftDisp,tutoDvBeam.d_rail.tutoDvBeam.disp}}

PA=dyn_ui('paramvh');projM=PA.nmap;
if ~isfield(PA,'ms')||isempty(PA.ms); d_rail('tutoDvBeam -s{init,mesh}');end

RO=struct('Range',struct('ncx',1./linspace(.05,.49,50)), ...
    'fmax',3e3,'ModalFilter',20, ...
...
RM2=cingui('paramedit -DoClean2','prero.railu.MeshSlice',{struct('half',0,'RailX',-300:50:300, ...
...
%mo1=d_mesh('beamtrack{ncell 1,lc .05,cyc}');
%mo1=d_rail('meshbeamMass{half0,cyc,lc.1}');
PA=dyn_ui('paramvh');projM=PA.nmap;

Range=struct('ncx',sdtm.urnValUG('@ll(2.01,20,40)'));
...
if 1==2
 RS=struct( ...
  'SubType','Spring', ...
  'SleeperType','Monoblock', ...
  'RailType','beam', ...
  'RailName','UIC60');
 list={'Slice',RS};
 r1=struct('ToolTip','Ballast only track without symmetry and custom rails', ...
    'value',{list});
 r2('RailBeam')=r1;
end

%% step dfrf : forced response
mo1=stack_set(mo1,'info','Freq',sdtm.urnValUG('@ll{1,3k,500}')');
mo1=fe_case(mo1,'DofLoad','In',struct('def',eye(2),'DOF',[1.03;1.02]));
...
RV=struct('hist',h2,'ktype','kc','doGetData',1);
RV.sens=fe_case(mo1,'sens');
RV.ci=2;RV.PlotInfo='2D-type"surface"';
RV.os={'@axes',{'colormap',ii_plp('colormapband',parula(5))}, ...
       '@OsDic',{'cbTR{string,log(vel)}'}};
...
RO.li={'Name','ToolTip','meta'
 ...
 'Attaches','line of attaches', ...
  struct('wd','28_05/*_Attaches','In','sX','Out','HamAcc','rep',6*ones(1,12*4) )
 'vZhf','metal vertical', ...
 ...
 'vZmf','plastique vertical', ...
  struct('wd','28_05/*_plast*vertical','In','vZ13','Out','HamAcc','rep',[5*ones(1,12) 10]) % xxxgae 12 pos 5 strike done later 
 'vYmf','plastique lateral', ...
  struct('wd','28_05/*_plast*lateral','In','vY13','Out','HamAcc','rep',5*ones(1,13)) % xxxgae 12 pos 5 strike done later 
 'vZlf','masse vertical', ...
...
if ~RO.cyc&&RO.ncell(1)>0
 if RO.half==3
  %% #Ballast_beam_random properties -4
  %mt=fe_case(mt,'fixdof','ends',sprintf('x==-.3|x==%g',.6*RO.ncell+.3));
  mt=fe_case(mt,'remove','ends'); % No ends to allow base mode
  mt=fe_case(mt,'fixdof','ends',sprintf('x==-.3|x==%g -DOF1',.6*RO.ncell+.3));
  C1=fe_mknl('init',mo1);
  i1=feutil('findelt matid 400',mt);
  if isempty(i1);error('Missing ballast');end
  pro=struct('type','p_beam','il',feutil('getil 400',mo1), ...
      'integ',C1.GroupInfo{1,3}, ...
      'constit',repmat(C1.GroupInfo{1,4},1,length(i1)));
  i1=pro.constit(1,:);coef=(1+(rand(size(i1))-.5)*RO.rand);
  
  pro.constit(1,:)=pro.constit(1,:).*coef;
  mt.il(mt.il(:,1)==400,:)=[];mt=stack_set(mt,'pro','RandomBallast',pro);
  [EGroup,nGroup]=getegroup(mt.Elt);% remove duplicate celas
  jGroup=3; cEGI= EGroup(jGroup)+1:EGroup(jGroup+1)-1;
  elt=unique(mt.Elt(cEGI,:),'rows');mt.Elt(cEGI(1:size(elt,1)),:)=elt;
  mt.Elt(cEGI(size(elt,1)+1:end),:)=[];
  sdtkey('cvsnum>1.238;','p_solid');
  r2=0:.6:3+1e-8; % Add input and output
  for j1=1:length(r2); 
   try; r2(j1)=feutil(sprintf('findnode %g 0 0',.6*fix(RO.ncell/2)+r2(j1)),mt);end
  end
  r2(rem(r2,1)~=0)=[];
  mt=fe_case(mt,'DofLoad','In',struct('def',1,'DOF',r2(1)+.03), ...
      'SensDof','Out',r2(:)+.03);
  mt=stack_set(mt,'info','Freq',logspace(log10(3),log10(500),200)');
  mt.name=sprintf('BeamBallast_rand%.2f',RO.rand);
 elseif ~RO.rand % No random
 else
 %% #Random spring ballast -4
  i1=feutil('findelt eltnamecelas',mt);i1=i1(mt.Elt(i1,7)==RO.kb);
  
  %mt.doRand=@(mt)mt.Elt.*(1+sparse(i1,7,(rand(size(i1))-.5)*RO.rand,size(mt.Elt,1),size(mt.Elt,2)));
  diag(sparse((1+(rand(size(mt.irand))-.5)*.3)))*mt.Elt(mt.irand,7);

  coef=(1+(rand(size(i1))-.5)*RO.rand);
  mt.Elt(i1,7:end)=diag(sparse(coef))*mt.Elt(i1,7:end);
  mt.irand=i1;
 end
 if RO.randp
 %% #Random pad -4
  i1=feutil('findelt eltnamecelas',mt);i1=i1(mt.Elt(i1,7)==RO.kp);  
  coef=(1+(rand(size(i1))-.5)*RO.randp);
  mt.Elt(i1,7:end)=diag(sparse(coef))*mt.Elt(i1,7:end);
 end
 if isfield(RO,'randm')&&RO.randm
 %% #Random mass -4
  i1=feutil('findelt eltnamecelas',mt);i1=i1(mt.Elt(i1,7)==RO.km);
  coef=(1+(rand(size(i1))-.5)*RO.randm);
  mt.Elt(i1,7:end)=diag(sparse(coef))*mt.Elt(i1,7:end);
 end

 if isfield(RO,'fix');mt=fe_case(mt,RO.fix{:});end
 if RO.Ftr
     if RO.half
        error('HalfTrack and FTR are not compatible options.') 
     end
  mt=feutil('joinall',mt);
  %mt=fe_case(mt,'fixdof','2DRail','matid301&x~=0 -DOF 1 2 4 6');
  mt=fe_case(mt,'remove','2DRail','remove','ends');
  mt=fn_cosim('convFtr',mt);
...
li={'prrLine',[399 fe_mat('p_beam','SI',1) 2e-12 1e-4 1e-4 1e-6];% small place holder beam
   ...
   %'UIC60beam',[301 fe_mat('p_beam','SI',1) 1e-5 3.0383e-5 5.123e-6 0.00767]; 

    'BallastA',[401 fe_mat('p_spring','SI',1) 50e6 0 0 0.05 0];

    % sdtu.f.open('@onedrive/*/sncf*/doc/R23_Stage_Hassan.pdf#page=43')
    'PadA', [501 fe_mat('p_spring','SI',1) 300e6 0 0 0.1 0];
    % Beams J I1 I2 A    hyp J=I1+I2
    % sdtu.f.open('@onedrive/*/sncf*/e*/21*/Sleeper_IN00213.pdf#page=116')
    % https://hal.science/hal-04894600/file/VJMech_21542-OnlineFirst.pdf#page=10
    % B2 = 8.13 MNm2, Msl=145.92 kg/m, SL=2.41 
    % Kfoundation : MNm-2 182.57, viscosity MNsm^2 24.4, 
...
%% #Latex #Tex #Hevea -------------------------------------------------------2
elseif comstr(Cam,'svg'); error('Use MathJax')    
...
cd(pw0);
sdtweb('_link','xxxhbm_utils(''helpputall'')');
sdtweb('_link','xxxsd helpcursync helphbm');
